...
use crate::multipath::{MultipathOut, SubscriberParams};
use anyhow::Context;
...
impl<O: AsyncWrite + Send + Unpin + 'static> Media<O> {
    ...
    pub async fn run(&mut self, mode: InitMode, subscriber_params: SubscriberParams) -> anyhow::Result<()> {
        let mut tracks = vec![];
